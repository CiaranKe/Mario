package competition.uu2013.robinbaumgarten.astar;

import java.util.ArrayList;
import java.util.List;

import competition.uu2013.robinbaumgarten.astar.level.Level;
import competition.uu2013.robinbaumgarten.astar.level.SpriteTemplate;
import competition.uu2013.robinbaumgarten.astar.sprites.BulletBill;
import competition.uu2013.robinbaumgarten.astar.sprites.Enemy;
import competition.uu2013.robinbaumgarten.astar.sprites.FireFlower;
import competition.uu2013.robinbaumgarten.astar.sprites.Fireball;
import competition.uu2013.robinbaumgarten.astar.sprites.FlowerEnemy;
import competition.uu2013.robinbaumgarten.astar.sprites.Mario;
import competition.uu2013.robinbaumgarten.astar.sprites.Mushroom;
import competition.uu2013.robinbaumgarten.astar.sprites.Shell;
import competition.uu2013.robinbaumgarten.astar.sprites.Sprite;
import competition.uu2013.robinbaumgarten.astar.sprites.SpriteContext;
...
public class LevelScene implements SpriteContext, Cloneable
{
    ...
    public boolean setLevelScene(byte[][] data)
    {
        ...
        for (int y = (MarioYInMap - HalfObsWidth), yUpdate = 0; y < (MarioYInMap + HalfObsWidth); y++, yUpdate++)
        {
            for (int x = (MarioXInMap - HalfObsWidth), xUpdate = 0; x < (MarioXInMap + HalfObsWidth); x++, xUpdate++)
            {
                if (x >=0 && x <= level.xExit && y >= 0 && y < level.height)
                {
                    /*
                    byte datum = data[yUpdate][xUpdate];

                    if (datum != 0 && datum != -10 && datum != 1 && yUpdate > lastEventX)
                    {
                        lastEventX = xUpdate;
                    }
                    if (datum != 0 && datum != 1)
                    {
                        if (heights[xUpdate] == 0)
                        {
                            heights[xUpdate] = y;
                        }
                    }
                    ...
                    if (datum != 1 && level.getBlock(x, y) != 14)
                        */
                        level.setBlock(x, y, data[yUpdate][xUpdate]);
                }
            }
        }
	...
	public boolean setEnemies(float[] enemies) 
	{
		...
		for (int i = 0; i < enemies.length; i += 3)
		{
	        ...
	        case(Sprite.KIND_GREEN_KOOPA_WINGED): 
	        	System.out.println("====================================================================");
            System.out.println("Orig: Type: " + (byte) kind +" X: " + x + " Y: " + y);
	        	type = Enemy.ENEMY_GREEN_KOOPA; winged = true; break;
